...
session_start();
 ...
 /**
 * Kontroler koji upravlja stranicom moderatorPage
 * Nasljedjuje osnovnu Controller klasu
 *
 *
 * @package mojMVC\app\controllers
 * @subpackage moderatorPage
 * @version 1.0
 * @since 20-05-2015
 * @author   Zeljko Krnjic <zeljko-10000@hotmail.com>
 **/
class ModeratorPage extends Controller
{
	/**
	* Index akcija kontrolera ModeratorPage
	* @access public
	*/
	public function index()
	{
		...
		{
			$this->model('User');
			$this->view('moderator_panel/moderatorPage');
		}
		else
		{
	...
	*/
		...
		if(Session::isModerator())
			{
				...
				$this->view('moderator_panel/showUsers', ['users' => $users,
													  'searchBy' => $searchBy,
	...
	/**
	* Akcija za brisanje odabranog korisnika preko id-a iz moderator panela
	* @access public
	* @param int $id
	*/
	public function deleteUser($id)
	{
		...
		if(Session::isModerator())
		{
			...
			header("Location: index.php?url=moderatorPage/showUsers");
		}
		else
		{
			header("Location: index.php");
	...
	/**
	* Akcija za dodavanje korisnika u bazu preko moderator panela.
	...
	*/
	public function addUser()
	{
		...
		if(Session::isModerator())
		{
			...
			$this->view('moderator_panel/addUser', ['errors' => $errors,
	...
	/**
	* Akcija za izmjenu podataka odabranog korisnika preko id-a iz moderator panela
	* @access public
	* @param int $id
	*/
	public function editUser($id='')
	{
		...
		$this->view('moderator_panel/editUser', ['user' => $user,
											 'errors' => $errors,
											 'errorStyle' => $errorStyle]);
	}
	...
	/**
	* Akcija za prikaz i pretragu rezervacija spremljenih u bazi iz moderator panela.
	* @access public
	* @param string $searchBy
	* @param string $searchKeyword
	*/
	public function pregledRezervacija($searchBy='', $searchKeyword='')
	{
		...
		if(!Session::isModerator()) header("Location index.php");
		$this->model('Session');
		...
		if(isset($_POST['searchRezervacijeButton']))
		{
			...
			if(!empty($searchBy) && !empty($searchKeyword))
			{
				...
				foreach($db->result as $rezervacija)
				{
		...
		$this->view('moderator_panel/pregledRezervacija', ['rezervacije' => $rezervacije,
													   'searchBy' => $searchBy,
													   'searchKeyword' => $searchKeyword]);
	}
	...
	*/
	public function brisiRezervaciju($id='')
	{
		...
		header("Location: index.php?url=moderatorPage/pregledRezervacija");
	}
}